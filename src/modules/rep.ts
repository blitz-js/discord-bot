import { command, default as CookiecordClient, Module, optional, listener } from "cookiecord"
...
export class RepModule extends Module {
  ...
  async getOrMakeUser(user: User) {
    let ru = await RepUser.findOne({ id: user.id }, { relations: ["got", "given"] })
  ...
  async onThank(msg: Message) {
    ...
    for (const user of mentionUsers) {
      ...
      if (currentSent >= this.MAX_REP) return await msg.react(PARTIAL_GIVE)
  ...
  async remaining(msg: Message) {
    ...
    await msg.channel.send(`Rep used: ${USED.repeat(sent) + UNUSED.repeat(this.MAX_REP - sent)}`)
  }
  ...
  async rep(msg: Message, targetMember: GuildMember) {
    ...
    if ((await senderRU.sent()) >= this.MAX_REP)
      return await msg.channel.send(":warning: no rep remaining! come back later.")
    ...
    await msg.channel.send(
      `:ok_hand: sent \`${targetMember.displayName}\` 1 rep (${(await senderRU.sent()) + 1}/${
        this.MAX_REP
      } sent)`
    )
  }
  ...
  async getrep(msg: Message, @optional user?: User) {
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        (
          await Promise.all(
            (await targetRU.got).concat(await targetRU.given).map(async (rg) => {
              if (rg.from.id == targetRU.id)
                return `:white_small_square: Gave 1 rep to <@${rg.to.id}> (${prettyMilliseconds(
                  Date.now() - rg.createdAt.getTime()
                )} ago)`
              else
                return `:white_small_square: Got 1 rep from <@${rg.from.id}> (${prettyMilliseconds(
                  Date.now() - rg.createdAt.getTime()
                )} ago)`
            })
          )
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        data
          .map(
            (x, index) =>
              `${topEmojis[index] || ":white_small_square:"} **<@${x.id}>** with **${
                x.count
              }** points.`
          )
          .join("\n")
