...
export class TwoslashModule extends Module {
  @command({
    single: true,
    description: "Search for a symbol and get the type of it in the latest codeblock",
  })
  async ts(msg: Message, content: string) {
    const match = /^[_$a-zA-Z][_$0-9a-zA-Z]*/.exec(content)

    if (!match) {
      return sendWithMessageOwnership(msg, "You need to give me a valid symbol name to look for!")
    }
    ...
    const value = ret.staticQuickInfos.find((i) => i.targetString === symbol.trim())
    if (!value)
      return sendWithMessageOwnership(
        msg,
  ...
  @command({
    description: "Run twoslash on the latest codeblock, returning compiler errors and queries",
  })
  ...
  private async twoslashBlock(msg: Message, code: string) {
    ...
    twoslashLines.forEach((line, index) => {
      ...
      if (lineErrors.length) {
        ...
        const errors = lineErrors.map((e) => "// " + e.renderedMessage.split("\n").join("\n// "))
    ...
    return sendWithMessageOwnership(msg, `${CODEBLOCK}ts\n${escapeCode(output)}${CODEBLOCK}\n`)
  }
}
