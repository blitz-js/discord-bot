import { command, default as CookiecordClient, listener, Module, optional } from "cookiecord"
import { Message, MessageEmbed, TextChannel } from "discord.js"
import { compressToEncodedURIComponent } from "lz-string"
import { TS_BLUE } from "../env"
import { findCodeblockFromChannel, PLAYGROUND_REGEX } from "../util/findCodeblockFromChannel"
...
export class PlaygroundModule extends Module {
  ...
  async playground(msg: Message, @optional code?: string) {
    ...
    if (!code) {
      code = await findCodeblockFromChannel(msg.channel as TextChannel, true)
      if (!code) return sendWithMessageOwnership(msg, ":warning: couldn't find a codeblock!")
    }
